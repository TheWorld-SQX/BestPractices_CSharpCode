...
### 有意识地去引导
服务端：
网页端数据查询患者相关胶片信息更新展示（给护士台用）：分页查询（数据库游标；linq；各个流行数据库自带的方法）；网页端用的什么什么…技术,都是哪些功能，比如…查询pacs患者基本原信息：患者胶片报告匹配状态；胶片报告打印状态；未识别的胶片报告；手动校对识别号码；胶片报告打印量统计；把一些第三方的识别库集成到识别引擎里；   
...
客户端：
扫码枪传入扫码号；调用服务端的api查询患者相关信息（类型，是不是急诊，普通， 住院，门诊，类型不一样打印权限也不一样）;返回查到的胶片报告相关，数量，能不能打印，信息通过一些文本语音模块TTL返回给患者；
同步打印状态,打印信息给服务器，跟第三方封装的收费接口webapi对接获取订单状态同步订单状态;
触发器检测状态；收费接口状态查询检测；
物联网模块，串口通信，AT命令启动三方nb卡，辅着到网络分配ip（设备状态有没有故障,有没有卡片）向服务器发送胶片打印信息；
扫码枪串口读取条码号，服务器接口查询结果，串口发送AT命令控制物联网模块依附着网络，向服务器上报终端机器状态（某个服务器接口）
